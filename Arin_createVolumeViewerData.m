...
%    dataDescriptor description of the data in the nc-file, e.g 'temp'
    ...
    %disp("zeta " +zeta);
    disp("vtransform " +Vtransform);
    disp("vstreching " +Vstretching);
    disp("theta_s " +theta_s);
    disp("theta_b " +theta_b);
    
    disp("hc " +hc);
    %disp("ocean_time " +ocean_time);
    ...
    %for time = 1:size(data,4)
        ...
        file = fopen([OutputFolder '/data/' dataDescriptor '_' OsomDataFileName '_timestep_1.raw'],'w');
        fwrite(file,outData,'float32');
        fclose(file);

        file = fopen([OutputFolder '/data/' dataDescriptor '_' OsomDataFileName '_timestep1.desc'],'w');
